...
setup_page_tables:
    ...
    ; Here, Identity mapping first 1GiB memory = 512-pages * 2MiB per page
    ...
    ; enabling `huge-page` flag on any try in Level-2 table enables to point directly to physical memory page of 2MiB, avoiding Level-1 table, offset = 9-bits + 12-bits
    mov ecx, 0  ; counter
.loop:
...
gdt64:  ; 64-bit global descriptor table
    dq 0    ; zero entry
.code_segement: equ $ - gdt64   ; offset to code-segment
    ; code segment
